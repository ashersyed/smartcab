...
class Simulator(object):
    ...
    def run(self, n_trials=1):
        ...
        for trial in xrange(n_trials):
            ...
            while True:
                try:
                    ...
                    # if we're leaving the game or a trial is done
                    if self.quit or self.env.done: 
                        # get the state of the primary agent from the environment
                        state = self.env.agent_states[self.env.primary_agent]
                        # if the agent didn't reach its destination    
                        if state['location'] != state['destination']:
                            # set last_dest_fail as this trial's number (plus 1 because the loop starts from 0) 
                            self.env.primary_agent.last_dest_fail = trial + 1
                        if state['location'] == state['destination']:
                            self.env.primary_agent.n_dest_reached += 1
                            self.env.primary_agent.sum_time_left += self.env.get_deadline(self.env.primary_agent)
                        # if the number of penalties increased during this trial                            
                        if self.env.primary_agent.n_penalties > prev_penalty:
                            # set last_penalty as this trial number 
        ...
        return self.env.primary_agent.reward_sum, self.env.primary_agent.disc_reward_sum, self.env.primary_agent.n_dest_reached, self.env.primary_agent.last_dest_fail, self.env.primary_agent.sum_time_left, self.env.primary_agent.n_penalties, self.env.primary_agent.last_penalty, self.env.primary_agent.len_qvals
